...
class TransactionData:
    ...
    @staticmethod
    async def other_common_data(currency: Currency, to_currency: Currency, sender_account: dict,
                                receiver_account: dict, amount: float, amount_to_receiver_account: float):
        return {
            ...
            'balance_receiver_account': {
                ...
                'balance_after_transaction': round(receiver_account['balance'], 2) + amount_to_receiver_account
            }
        }
    ...
    async def get_deposit_or_withdraw_data(amount: float, transaction_type: TransactionType,
        ...
        return {
            ...
            'currency': currency,
            'description': None,
            'amount_to_receiver_account': None,
            **{
                'is_atm': True,
    ...
    async def get_transfer_data(self, amount: float, currency: Currency, desc: Optional[str],
                                sender_account: dict, receiver_account: dict,
        ...
        other_data = await self.other_common_data(currency=currency, to_currency=currency,
    ...
    async def get_exchange_data(self, amount: float, amount_to_receiver_account: float,
        ...
        data = await self.get_common_data(
            amount=-amount, amount_to_receiver_account=amount_to_receiver_account, description=description,
            currency=currency, transaction_type=transaction_type, sender_account=sender_account,
            receiver_account=receiver_account)
